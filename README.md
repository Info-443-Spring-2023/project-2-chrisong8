...
|Refactoring: Extract Constant| In `packages/src/api/Database.ts`, the constant value 'FIREBASE_DATABASE_EMULATOR_HOST' lacks a meaningful variable name. In order to recitfy that, a new variable was declared with a more meaningful name of `firebaseDatabaseEmulatorHost`. All occurrences of the constant was then replaced with the new variable. This will improve the code's readability and maintainability since the constant now has a descriptive name that accurately conveys its purpose. By extracting the constant into a seperate variable with a more meaningful name, comprehension of the code is improved. 
|--|--|